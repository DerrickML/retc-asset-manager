...
export class EmailService {
  ...
  static async sendSystemAlert(alertType, severity, details, branding = null) {
    ...
    await this.sendNotification(EMAIL_TYPES.SYSTEM_ALERT, "admins", data, branding)
  }
  ...
  static async sendRequestApproved(request, requester, asset, approver) {
    ...
    await this.sendNotification(EMAIL_TYPES.REQUEST_APPROVED, requester.email, data)
  }
  ...
  static async sendRequestDenied(request, requester, asset, approver) {
    ...
    await this.sendNotification(EMAIL_TYPES.REQUEST_DENIED, requester.email, data)
  }
  ...
  static async sendAssetIssued(request, requester, asset, issuer) {
    ...
    await this.sendNotification(EMAIL_TYPES.ASSET_ISSUED, requester.email, data)
  }
  ...
  static async sendAssetAssigned(asset, custodian, assigner, notes = null) {
    ...
    await this.sendNotification(EMAIL_TYPES.ASSET_ASSIGNED, custodian.email, data)
  }
  ...
  static async sendReturnReminder(request, requester, asset) {
    const data = {
      ...
      requestId: request.$id,
      expectedReturnDate: request.expected_return_date,
      daysUntilDue: Math.ceil((new Date(request.expected_return_date) - new Date()) / (1000 * 60 * 60 * 24)),
    }

    await this.sendNotification(EMAIL_TYPES.RETURN_REMINDER, requester.email, data)
  }
  ...
  static async sendReturnOverdue(request, requester, asset) {
    const data = {
      ...
      requestId: request.$id,
      expectedReturnDate: request.expected_return_date,
      daysOverdue: Math.ceil((new Date() - new Date(request.expected_return_date)) / (1000 * 60 * 60 * 24)),
    }

    // Send to requester and admins
    await this.sendNotification(EMAIL_TYPES.RETURN_OVERDUE, requester.email, data)
    await this.sendNotification(EMAIL_TYPES.RETURN_OVERDUE, "admins", data)
  }

  static async sendMaintenanceDue(asset, assignedTechnician) {
    const data = {
      assetName: asset.name,
      assetTag: asset.assetTag,
      assetId: asset.$id,
      ...
      technicianName: assignedTechnician?.name || "Maintenance Team",
      technicianEmail: assignedTechnician?.email,
      locationName: asset.locationName,
      roomOrArea: asset.roomOrArea,
      currentCondition: asset.currentCondition,
      daysUntilDue: Math.ceil((new Date(asset.nextMaintenanceDue) - new Date()) / (1000 * 60 * 60 * 24)),
    }

    const recipient = assignedTechnician?.email || "admins"
    await this.sendNotification(EMAIL_TYPES.MAINTENANCE_DUE, recipient, data)
  }
