...
export class DashboardService {
  ...
  static async getDashboardMetrics(
    ...
    try {
      ...
      const metrics = this._calculateMetrics(
        ...
        requestsResult.documents,
        staffResult.documents
      );
  ...
  static _calculateMetrics(assets, requests, staff) {
    ...
    for (const asset of actualAssets) {
      ...
      metrics.assetsByCategory[category] =
        (metrics.assetsByCategory[category] || 0) + 1;

      // Count by condition
      const condition = asset.currentCondition || "Unknown";
      metrics.assetsByCondition[condition] =
        (metrics.assetsByCondition[condition] || 0) + 1;
    }
    ...
    for (const request of requests) {
      ...
      metrics.requestsByStatus[request.status] =
        (metrics.requestsByStatus[request.status] || 0) + 1;
    }

    // Count active users (those with custodian assignments)
    const activeCustodians = new Set(
      assets.map((a) => a.custodianStaffId).filter(Boolean)
    );
  ...
  static _calculateAnalytics(assets, events) {
    ...
    for (const asset of assets) {
      ...
      if (asset.purchasePrice) {
        ...
        analytics.costAnalysis.valueByCategory[category] =
          (analytics.costAnalysis.valueByCategory[category] || 0) +
          parseFloat(asset.purchasePrice);
      }
    }

    // Convert category map to array with percentages
    analytics.categoryDistribution = Object.entries(categoryMap).map(
      ([name, value]) => ({
        name,
        value,
    ...
    analytics.costAnalysis.avgAssetValue =
      assets.length > 0 ? totalValue / assets.length : 0;
    ...
    for (const asset of assets) {
      ...
      if (
        [
          ...
          ENUMS.CURRENT_CONDITION.POOR,
        ].includes(asset.currentCondition)
      ) {
      ...
      if (
        [
          ...
          ENUMS.AVAILABLE_STATUS.AWAITING_DEPLOY,
        ].includes(asset.availableStatus) &&
        !asset.custodianStaffId
      ) {
  ...
  static _calculateDepartmentUtilization(departments, assets) {
    ...
    for (const dept of departments) {
      ...
      const utilizationRate =
        deptAssets.total > 0 ? (deptAssets.inUse / deptAssets.total) * 100 : 0;
