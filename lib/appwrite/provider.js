...
export const assetsService = {
  ...
  async adjustConsumableStock(id, adjustment, actorStaffId, notes = null) {
    ...
    // Extract current stock from serialNumber field (stored as "STOCK:123")
    const currentStock =
      consumable.serialNumber && consumable.serialNumber.startsWith("STOCK:")
        ? parseInt(consumable.serialNumber.replace("STOCK:", "")) || 0
        : 0;

    // Extract min stock from model field (stored as "MIN:123")
    const minStock =
      consumable.model && consumable.model.startsWith("MIN:")
        ? parseInt(consumable.model.replace("MIN:", "")) || 0
        : 0;
    ...
    // Update the encoded fields
    const updateData = {
      serialNumber: `STOCK:${newStock}`,
    };

    // Update subcategory if it exists, preserving the format "UNIT|STATUS|CATEGORY"
    if (consumable.subcategory && consumable.subcategory.includes("|")) {
      const parts = consumable.subcategory.split("|");
      if (parts.length >= 3) {
        parts[1] = newStatus; // Update the status part
        updateData.subcategory = parts.join("|");
      } else {
        // Fallback if format is unexpected
        updateData.subcategory = `${parts[0] || ""}|${newStatus}|${
          parts[2] || ""
        }`;
      }
    } else {
      // If no subcategory exists, create a basic one
      updateData.subcategory = `|${newStatus}|`;
    }
