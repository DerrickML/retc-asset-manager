...
export async function writeAssetEvent(
  ...
  try {
    ...
    console.log("Writing asset event:", eventData)
...
export const assetsService = {
  async list(queries = []) {
    const result = await databases.listDocuments(APPWRITE_CONFIG.databaseId, COLLECTIONS.ASSETS, queries)
  ...
  async create(data, actorStaffId) {
    ...
    console.log("Creating asset with data:", preparedData)

    const asset = await databases.createDocument(APPWRITE_CONFIG.databaseId, COLLECTIONS.ASSETS, "unique()", preparedData)

    // Write creation event
    if (actorStaffId) {
      await writeAssetEvent(asset.$id, ENUMS.EVENT_TYPE.CREATED, null, preparedData.currentCondition, actorStaffId, "Asset created")
    }
  ...
  async update(id, data, actorStaffId, notes = null, options = {}) {
    ...
    const preparedData = {
      ...data,
      attachmentFileIds: data.attachmentFileIds !== undefined ? data.attachmentFileIds : undefined,
    }
    ...
    const updatedAsset = await databases.updateDocument(APPWRITE_CONFIG.databaseId, COLLECTIONS.ASSETS, id, preparedData)

    // Get actor staff details for notifications
    const actorStaff = actorStaffId ? await staffService.get(actorStaffId).catch(() => null) : null
    ...
    if (oldAsset.custodianStaffId !== data.custodianStaffId && data.custodianStaffId) {
      try {
        const newCustodian = await staffService.get(data.custodianStaffId)
        if (newCustodian && newCustodian.email && options.sendNotification !== false) {
          await EmailService.sendAssetAssigned(updatedAsset, newCustodian, actorStaff, notes)
        }
      } catch (error) {
    ...
    if (data.nextMaintenanceDue && options.checkMaintenanceDue !== false) {
      ...
      const now = new Date()
      const daysUntilMaintenance = Math.ceil((maintenanceDueDate - now) / (1000 * 60 * 60 * 24))
      
      // Send notification if maintenance is due within 7 days
      if (daysUntilMaintenance <= 7 && daysUntilMaintenance >= 0) {
        try {
          const custodian = data.custodianStaffId ? await staffService.get(data.custodianStaffId).catch(() => null) : null
...
export const assetRequestsService = {
  ...
  async getByStaff(staffId) {
    return await databases.listDocuments(APPWRITE_CONFIG.databaseId, COLLECTIONS.ASSET_REQUESTS, [
  ...
  async create(data, options = {}) {
    const request = await databases.createDocument(APPWRITE_CONFIG.databaseId, COLLECTIONS.ASSET_REQUESTS, "unique()", data)

    // Send notification to admins about new request
    if (options.sendNotification !== false) {
      try {
        await EmailService.sendRequestSubmitted(request, options.requester, options.asset)
      } catch (error) {
  ...
  async update(id, data, options = {}) {
    ...
    const updatedRequest = await databases.updateDocument(APPWRITE_CONFIG.databaseId, COLLECTIONS.ASSET_REQUESTS, id, data)

    // Send notifications based on status changes
    if (options.sendNotification !== false && oldRequest.status !== data.status) {
      try {
        switch (data.status) {
          case ENUMS.REQUEST_STATUS.APPROVED:
            if (options.requester && options.asset && options.approver) {
              await EmailService.sendRequestApproved(updatedRequest, options.requester, options.asset, options.approver)
            }
            break
          case ENUMS.REQUEST_STATUS.DENIED:
            if (options.requester && options.asset && options.approver) {
              await EmailService.sendRequestDenied(updatedRequest, options.requester, options.asset, options.approver)
            }
            break
          case ENUMS.REQUEST_STATUS.FULFILLED:
            if (options.requester && options.asset && options.issuer) {
              await EmailService.sendAssetIssued(updatedRequest, options.requester, options.asset, options.issuer)
            }
            break
        }
      } catch (error) {
...
export const assetIssuesService = {
  ...
  async create(data) {
    console.log("Creating asset issue with data:", data)
...
export const assetReturnsService = {
  ...
  async create(data, options = {}) {
    const returnRecord = await databases.createDocument(APPWRITE_CONFIG.databaseId, COLLECTIONS.ASSET_RETURNS, "unique()", data)
    
    // Send email notification about asset return if enabled
    if (options.sendNotification !== false && options.request && options.requester && options.asset) {
      try {
        await EmailService.sendAssetReturned(options.request, options.requester, options.asset, data.returnCondition)
      } catch (error) {
...
export const assetEventsService = {
  async list(queries = []) {
    const result = await databases.listDocuments(APPWRITE_CONFIG.databaseId, COLLECTIONS.ASSET_EVENTS, queries)
  ...
  async getByAssetId(assetId) {
    const result = await databases.listDocuments(APPWRITE_CONFIG.databaseId, COLLECTIONS.ASSET_EVENTS, [
