...
export default function AdminDashboard() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const assetMetrics = {
        ...
        availableAssets: assets.documents.filter((a) => a.status === "Available").length,
        inUseAssets: assets.documents.filter((a) => a.status === "In Use").length,
        maintenanceAssets: assets.documents.filter((a) => a.status === "Maintenance").length,
        pendingRequests: requests.documents.filter((r) => r.status === "Pending").length,
  ...
  const COLORS = ["#059669", "#0891b2", "#7c3aed", "#dc2626", "#ea580c", "#65a30d"]
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={requestTrends}>
                    ...
                    <YAxis />
                    <Tooltip />
                    <Line type="monotone" dataKey="requests" stroke="#059669" strokeWidth={2} />
                    <Line type="monotone" dataKey="approved" stroke="#0891b2" strokeWidth={2} />
                  </LineChart>
                </ResponsiveContainer>
              </CardContent>
            </Card>
