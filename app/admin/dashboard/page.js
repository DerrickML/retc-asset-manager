  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const metrics = {
        ...
        availableAssets: assets.filter(a => a.availableStatus === ENUMS.AVAILABLE_STATUS.AVAILABLE).length,
        inUseAssets: assets.filter(a => a.availableStatus === ENUMS.AVAILABLE_STATUS.IN_USE).length,
        maintenanceAssets: assets.filter(a => 
          a.availableStatus === ENUMS.AVAILABLE_STATUS.MAINTENANCE || 
          a.availableStatus === ENUMS.AVAILABLE_STATUS.REPAIR_REQUIRED
        ).length,
        retiredAssets: assets.filter(a => 
      ...
      const assetsByCategory = Object.entries(categoryMap).map(([category, count]) => ({
        name: category.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, l => l.toUpperCase()),
        value: count,
        percentage: assets.length > 0 ? ((count / assets.length) * 100).toFixed(1) : 0
      }))

      // Process assets by status (real data)
      const statusMap = {}
      Object.values(ENUMS.AVAILABLE_STATUS).forEach(status => {
        statusMap[status] = assets.filter(a => a.availableStatus === status).length
      })
      ...
      Object.values(ENUMS.REQUEST_STATUS).forEach(status => {
        requestStatusMap[status] = requests.filter(r => r.status === status).length
      })
      ...
      departments.forEach(dept => {
        ...
        if (deptAssets.length > 0) {
          deptAssetsMap[dept.name] = {
            ...
            available: deptAssets.filter(a => a.availableStatus === ENUMS.AVAILABLE_STATUS.AVAILABLE).length,
            inUse: deptAssets.filter(a => a.availableStatus === ENUMS.AVAILABLE_STATUS.IN_USE).length,
            utilization: deptAssets.length > 0 ? 
              ((deptAssets.filter(a => a.availableStatus === ENUMS.AVAILABLE_STATUS.IN_USE).length / deptAssets.length) * 100).toFixed(1) 
              : 0
          }
        }
      })

      const assetsByDepartment = Object.entries(deptAssetsMap).map(([deptName, data]) => ({
        name: deptName,
        ...data
      }))
        ...
        <div className="bg-white/70 backdrop-blur-sm rounded-2xl border border-white/50 shadow-lg shadow-blue-500/5 p-6">
          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center space-y-4 lg:space-y-0">
            <div className="space-y-1">
              ...
              <p className="text-slate-600 font-medium">Real-time system analytics & insights</p>
            ...
            <div className="flex flex-wrap gap-3">
              <Button 
                ...
                variant="outline" 
                disabled={refreshing}
                className="bg-white/80 border-blue-200 hover:bg-blue-50 hover:border-blue-300 transition-all duration-200"
              >
          ...
          <Card className="bg-gradient-to-br from-blue-50 to-indigo-100 border-0 shadow-lg hover:shadow-xl transition-all duration-300 group">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge className="bg-blue-100 text-blue-700 hover:bg-blue-200 border-0">Total</Badge>
              </div>
              <div className="space-y-1">
                <h3 className="text-3xl font-bold text-slate-900">{dashboardData.metrics.totalAssets}</h3>
                <p className="text-sm font-medium text-slate-600">Total Assets</p>
                <p className="text-xs text-blue-600">
                  {dashboardData.metrics.availableAssets} available • {dashboardData.metrics.inUseAssets} in use
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gradient-to-br from-emerald-50 to-green-100 border-0 shadow-lg hover:shadow-xl transition-all duration-300 group">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge className="bg-green-100 text-green-700 hover:bg-green-200 border-0">Ready</Badge>
              </div>
              <div className="space-y-1">
                <h3 className="text-3xl font-bold text-slate-900">{dashboardData.metrics.availableAssets}</h3>
          ...
          <Card className="bg-gradient-to-br from-amber-50 to-orange-100 border-0 shadow-lg hover:shadow-xl transition-all duration-300 group">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge className="bg-orange-100 text-orange-700 hover:bg-orange-200 border-0">Pending</Badge>
              </div>
              <div className="space-y-1">
                <h3 className="text-3xl font-bold text-slate-900">{dashboardData.metrics.pendingRequests}</h3>
          ...
          <Card className="bg-gradient-to-br from-red-50 to-rose-100 border-0 shadow-lg hover:shadow-xl transition-all duration-300 group">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge className="bg-red-100 text-red-700 hover:bg-red-200 border-0">Alert</Badge>
              </div>
              <div className="space-y-1">
                <h3 className="text-3xl font-bold text-slate-900">{dashboardData.metrics.maintenanceAssets}</h3>
          ...
          <Card className="bg-gradient-to-br from-purple-50 to-indigo-100 border-0 shadow-lg hover:shadow-xl transition-all duration-300 group">
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Badge className="bg-purple-100 text-purple-700 hover:bg-purple-200 border-0">Team</Badge>
              </div>
              <div className="space-y-1">
                <h3 className="text-3xl font-bold text-slate-900">{dashboardData.metrics.totalStaff}</h3>
            ...
            <div className="flex items-start space-x-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-orange-800 mb-3">
                  You have <span className="font-bold text-orange-900">{dashboardData.metrics.pendingRequests}</span> pending requests 
        ...
        <Tabs defaultValue="overview" className="space-y-8">
          <div className="bg-white/70 backdrop-blur-sm rounded-2xl border border-white/50 shadow-lg p-2">
            <TabsList className="grid w-full grid-cols-4 bg-transparent gap-1">
              <TabsTrigger 
                value="overview" 
                className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-blue-500 data-[state=active]:to-indigo-600 data-[state=active]:text-white data-[state=active]:shadow-lg hover:bg-blue-50 transition-all duration-200 rounded-xl font-medium"
              >
              ...
              <TabsTrigger 
                value="assets" 
                className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-emerald-500 data-[state=active]:to-green-600 data-[state=active]:text-white data-[state=active]:shadow-lg hover:bg-emerald-50 transition-all duration-200 rounded-xl font-medium"
              >
              ...
              <TabsTrigger 
                value="requests" 
                className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-orange-500 data-[state=active]:to-amber-600 data-[state=active]:text-white data-[state=active]:shadow-lg hover:bg-orange-50 transition-all duration-200 rounded-xl font-medium"
              >
              ...
              <TabsTrigger 
                value="activity" 
                className="data-[state=active]:bg-gradient-to-r data-[state=active]:from-purple-500 data-[state=active]:to-indigo-600 data-[state=active]:text-white data-[state=active]:shadow-lg hover:bg-purple-50 transition-all duration-200 rounded-xl font-medium"
              >
              ...
              <Card className="bg-white/70 backdrop-blur-sm border border-white/50 shadow-lg hover:shadow-xl transition-all duration-300">
                <CardHeader className="pb-6">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <CardTitle className="text-lg font-semibold text-slate-900">Asset Distribution by Category</CardTitle>
                      <CardDescription className="text-slate-600">
                        Real data from your <span className="font-medium text-blue-600">{dashboardData.metrics.totalAssets}</span> assets
                      </CardDescription>
                    </div>
                  </div>
                </CardHeader>
                  ...
                  <div className="space-y-4">
                    {dashboardData.assetsByCategory.length > 0 ? (
                      dashboardData.assetsByCategory.map((category, index) => (
                        <div key={category.name} className="group p-4 rounded-xl bg-slate-50/50 hover:bg-slate-100/50 transition-all duration-200">
              ...
              <Card className="bg-white/70 backdrop-blur-sm border border-white/50 shadow-lg hover:shadow-xl transition-all duration-300">
                <CardHeader className="pb-6">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <CardTitle className="text-lg font-semibold text-slate-900">Department Asset Utilization</CardTitle>
                      <CardDescription className="text-slate-600">Asset usage efficiency by department</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {dashboardData.assetsByDepartment.length > 0 ? (
                      dashboardData.assetsByDepartment.map((dept, index) => (
                        <div key={dept.name} className="group p-4 rounded-xl bg-slate-50/50 hover:bg-slate-100/50 transition-all duration-200">
                      ...
                      <div className="text-center py-12">
                        ...
                        <p className="text-slate-500 font-medium">No department assignments found</p>
                      </div>
                    )}
                  </div>
                </CardContent>
            ...
            <Card className="bg-white/70 backdrop-blur-sm border border-white/50 shadow-lg hover:shadow-xl transition-all duration-300">
              <CardHeader className="pb-6">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <CardTitle className="text-lg font-semibold text-slate-900">System Overview</CardTitle>
                    <CardDescription className="text-slate-600">Current system status and utilization metrics</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  <div className="text-center p-4 rounded-xl bg-gradient-to-br from-green-50 to-emerald-100 hover:from-green-100 hover:to-emerald-200 transition-all duration-200">
                    ...
                    <div className="text-xs text-green-600 font-medium">
                      {dashboardData.metrics.totalAssets > 0 
                        ? ((dashboardData.metrics.availableAssets / dashboardData.metrics.totalAssets) * 100).toFixed(1) 
                        : 0}% of total
                    </div>
                  </div>
                  <div className="text-center p-4 rounded-xl bg-gradient-to-br from-blue-50 to-indigo-100 hover:from-blue-100 hover:to-indigo-200 transition-all duration-200">
                    <div className="flex items-center justify-center mb-2">
                      ...
                      <Badge className="bg-blue-100 text-blue-700 border-0 text-xs">Active</Badge>
                    </div>
                    ...
                    <div className="text-xs text-blue-600 font-medium">
                      {dashboardData.metrics.totalAssets > 0 
                        ? ((dashboardData.metrics.inUseAssets / dashboardData.metrics.totalAssets) * 100).toFixed(1) 
                        : 0}% utilization
                    </div>
                  </div>
                  <div className="text-center p-4 rounded-xl bg-gradient-to-br from-orange-50 to-amber-100 hover:from-orange-100 hover:to-amber-200 transition-all duration-200">
                    <div className="flex items-center justify-center mb-2">
                      ...
                      <Badge className="bg-orange-100 text-orange-700 border-0 text-xs">Service</Badge>
                    </div>
                  ...
                  <div className="text-center p-4 rounded-xl bg-gradient-to-br from-slate-50 to-gray-100 hover:from-slate-100 hover:to-gray-200 transition-all duration-200">
                    <div className="flex items-center justify-center mb-2">
                      ...
                      <Badge className="bg-slate-100 text-slate-700 border-0 text-xs">Retired</Badge>
                    </div>
            ...
            <Card className="bg-white/70 backdrop-blur-sm border border-white/50 shadow-lg hover:shadow-xl transition-all duration-300">
              <CardHeader className="pb-6">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <CardTitle className="text-lg font-semibold text-slate-900">Asset Status Breakdown</CardTitle>
                    <CardDescription className="text-slate-600">Detailed status distribution of all assets</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {dashboardData.assetsByStatus.map((statusGroup, index) => {
                    const getStatusColor = (status) => {
                      switch(status) {
                        case 'AVAILABLE': return 'from-green-50 to-emerald-100 border-green-200'
                        case 'IN_USE': return 'from-blue-50 to-indigo-100 border-blue-200'
                        case 'MAINTENANCE': 
                        case 'REPAIR_REQUIRED': return 'from-orange-50 to-amber-100 border-orange-200'
                        case 'RETIRED':
                        case 'DISPOSED': return 'from-slate-50 to-gray-100 border-slate-200'
                        default: return 'from-purple-50 to-violet-100 border-purple-200'
                      }
                    }
                    ...
                    const getStatusIcon = (status) => {
                      switch(status) {
                        ...
                        case 'IN_USE': return <Zap className="w-5 h-5 text-blue-600" />
                        case 'MAINTENANCE': 
                        case 'REPAIR_REQUIRED': return <Settings className="w-5 h-5 text-orange-600" />
                        case 'RETIRED':
                        case 'DISPOSED': return <Archive className="w-5 h-5 text-slate-600" />
                        default: return <AlertCircle className="w-5 h-5 text-purple-600" />
                      }
                    }

                    return (
                      <div key={statusGroup.name} className={`p-6 bg-gradient-to-br ${getStatusColor(statusGroup.status)} rounded-2xl border shadow-lg hover:shadow-xl transition-all duration-300 group`}>
                        <div className="flex justify-between items-start mb-4">
                          <div className="flex items-center space-x-2">
                            {getStatusIcon(statusGroup.status)}
                            <h4 className="font-semibold text-slate-800">{statusGroup.name}</h4>
                          </div>
              ...
              <Card className="bg-white/70 backdrop-blur-sm border border-white/50 shadow-lg hover:shadow-xl transition-all duration-300">
                <CardHeader className="pb-6">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <CardTitle className="text-lg font-semibold text-slate-900">Request Status Summary</CardTitle>
                      <CardDescription className="text-slate-600">Current status of all asset requests</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                    ...
                    {dashboardData.requestsByStatus.length > 0 ? (
                      dashboardData.requestsByStatus.map((statusGroup, index) => {
                        const getRequestStatusColor = (status) => {
                          switch(status) {
                            case 'PENDING': return 'from-orange-50 to-amber-100 border-orange-200'
                            case 'APPROVED': return 'from-green-50 to-emerald-100 border-green-200'  
                            case 'FULFILLED': return 'from-blue-50 to-indigo-100 border-blue-200'
                            case 'REJECTED': return 'from-red-50 to-rose-100 border-red-200'
                            default: return 'from-slate-50 to-gray-100 border-slate-200'
                          }
                        }
                        ...
                        const getRequestStatusIcon = (status) => {
                          switch(status) {
                            ...
                            case 'FULFILLED': return <Zap className="w-5 h-5 text-blue-600" />
                            case 'REJECTED': return <XCircle className="w-5 h-5 text-red-600" />
                            default: return <Pause className="w-5 h-5 text-slate-600" />
                          }
                        }

                        return (
                          <div key={statusGroup.name} className={`p-4 bg-gradient-to-r ${getRequestStatusColor(statusGroup.status)} rounded-xl border hover:shadow-md transition-all duration-200`}>
                            <div className="flex items-center justify-between">
                              <div className="flex items-center space-x-3">
                                {getRequestStatusIcon(statusGroup.status)}
                                <div>
                                  <div className="font-semibold text-slate-800">{statusGroup.name}</div>
                                  <div className="text-sm text-slate-600">{statusGroup.percentage}% of total</div>
                                </div>
                              </div>
                              <div className="text-2xl font-bold text-slate-900">{statusGroup.value}</div>
                            </div>
                          </div>
                        )
                      })
              ...
              <Card className="bg-white/70 backdrop-blur-sm border border-white/50 shadow-lg hover:shadow-xl transition-all duration-300">
                <CardHeader className="pb-6">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <CardTitle className="text-lg font-semibold text-slate-900">Request Metrics</CardTitle>
                      <CardDescription className="text-slate-600">Key request processing metrics</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="space-y-6">
                    <div className="text-center p-6 bg-gradient-to-br from-orange-50 to-amber-100 rounded-2xl border border-orange-200">
                      ...
                      <div className="text-sm font-medium text-orange-600">Pending Approval</div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className="text-center p-4 bg-gradient-to-br from-blue-50 to-indigo-100 rounded-xl border border-blue-200">
                        ...
                        <div className="text-xs font-medium text-blue-600">Fulfilled</div>
                      </div>
                    </div>
                  </div>
                </CardContent>
            ...
            <Card className="bg-white/70 backdrop-blur-sm border border-white/50 shadow-lg hover:shadow-xl transition-all duration-300">
              <CardHeader className="pb-6">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <CardTitle className="text-lg font-semibold text-slate-900">Recent System Activity</CardTitle>
                    <CardDescription className="text-slate-600">Latest events from the audit trail</CardDescription>
                  </div>
                </div>
              </CardHeader>
              <CardContent>
                  ...
                  {dashboardData.recentEvents.length > 0 ? (
                    dashboardData.recentEvents.map((event, index) => {
                      const getEventColor = (eventType) => {
                        if (eventType.includes('CREATE')) return 'from-green-50 to-emerald-100 border-green-200'
                        if (eventType.includes('UPDATE')) return 'from-blue-50 to-indigo-100 border-blue-200'
                        if (eventType.includes('DELETE')) return 'from-red-50 to-rose-100 border-red-200'
                        if (eventType.includes('ASSIGN')) return 'from-purple-50 to-violet-100 border-purple-200'
                        return 'from-slate-50 to-gray-100 border-slate-200'
                      }

                      const getEventIcon = (eventType) => {
                        if (eventType.includes('CREATE')) return <CheckCircle2 className="h-5 w-5 text-green-600" />
                        if (eventType.includes('UPDATE')) return <Settings className="h-5 w-5 text-blue-600" />
                        if (eventType.includes('DELETE')) return <XCircle className="h-5 w-5 text-red-600" />
                        if (eventType.includes('ASSIGN')) return <Users className="h-5 w-5 text-purple-600" />
                        return <Activity className="h-5 w-5 text-slate-600" />
                      }

                      return (
                        <div key={event.$id} className={`p-4 bg-gradient-to-r ${getEventColor(event.eventType)} rounded-xl border hover:shadow-md transition-all duration-200 group`}>
                          <div className="flex items-start space-x-4">
                            ...
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center space-x-2 mb-2">
                                ...
                                <span className="text-xs text-slate-500 font-medium">
                                  {new Date(event.at).toLocaleDateString()} • {new Date(event.at).toLocaleTimeString()}
                                </span>
                              </div>
                    ...
                    <div className="text-center py-16">
                      ...
                      <p className="text-slate-500">System activity will appear here when events occur</p>
                    </div>
                  )}
                </div>
              </CardContent>
